...
export function FleetBoard({ issues, epicCosts, onPipelineAction, agentRunning, pendingEpicId }: FleetBoardProps) {
  ...
  return (
    <div className="flex flex-col flex-1">
      ...
      {/* Board: empty state when filters produce no results (factory-core-cur.1.14) */}
      {apps.length === 0 && (shipTypeFilter !== "all" || waveFilter !== null) ? (
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <p className="text-gray-400 text-sm mb-2">No epics match the current filters</p>
            <button
              onClick={() => {
                setShipTypeFilter("all");
                saveShipTypeFilter("all");
                setWaveFilter(null);
              }}
